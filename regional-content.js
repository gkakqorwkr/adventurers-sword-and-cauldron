(() => {
  ...
  function openBook(ui, game) {
    ...
    const section = region => {
      ...
      return `<section class="regional-section"><h3>${region.name}</h3><p class="regional-count">마물 ${seenMonsters.length}/7 · 요리 ${seenRecipes.length}/10</p><h4>발견한 마물</h4><div class="regional-grid">${monsterCards}</div><h4>발견한 요리</h4><div class="regional-grid">${recipeCards}</div></section>`;
    };
