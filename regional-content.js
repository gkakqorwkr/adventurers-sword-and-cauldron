(() => {
  const regions = [
    { id: "mistwood", name: "안개 낀 숲길", description: "젖은 흙길과 희미한 등불이 이어진다. 발자국과 약초, 위험이 모두 엷은 안개 속에 섞여 있다.", background: "assets/backgrounds/mistwood.png", monsters: [
    ...
    { id: "swamp", name: "고목 습지", description: "기울어진 고목 사이로 검푸른 물이 고여 있다. 갈대가 스치는 소리와 습한 흙냄새가 길을 감싼다.", background: "assets/backgrounds/swamp.png", monsters: [
    ...
    { id: "prairie", name: "잿불 초원", description: "식지 않은 재가 바람을 따라 흩날린다. 붉은 풀밭 너머로 불씨가 별처럼 깜빡인다.", background: "assets/backgrounds/prairie.png", monsters: [
    ...
    { id: "mine", name: "폐광 심층", description: "금 간 갱도와 푸른 광맥이 깊은 어둠으로 이어진다. 멀리서 곡괭이 같은 울림이 들려온다.", background: "assets/backgrounds/mine.png", monsters: [
    ...
    { id: "canyon", name: "재의 협곡", description: "검은 절벽 사이로 뜨거운 바람이 흐른다. 화산재가 빛을 가리며 붉은 협곡의 길을 덮는다.", background: "assets/backgrounds/canyon.png", monsters: [
  ...
  function boot() {
    ...
    game.render = () => {
      ...
      if (scene) {
        scene.dataset.region = isCamp ? "camp" : region.id;
        if (!game.enemy && !isCamp) scene.innerHTML = `<h2>${escape(region.name)}</h2><p>${escape(region.description)}</p>`;
      }
      const regionalMonster = monsters.find(monster => monster.name === game.enemy?.name);
      const image = game.enemy?.image || regionalMonster?.image;
      if (image && scene) {
        const heading = scene.querySelector("h2");
        if (heading && !scene.querySelector(".scene-monster-art")) heading.insertAdjacentHTML("beforebegin", `<img class="scene-monster-art" src="${image}" alt="${escape(game.enemy?.name || regionalMonster?.name || "마물")}" />`);
      }
    };
