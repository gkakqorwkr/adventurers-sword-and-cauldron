(() => {
  ...
  Object.assign(GEAR, {
    ...
    hornKingSpear:{name:"뿔왕의 가시창",icon:"🔱",slot:"weapon",label:"보스 고유 무기",mods:{strength:3,agility:1},price:320,description:"안개숲의 뿔왕에게서만 얻는, 돌진의 힘을 담은 가시창.",unique:true},
    myceliumCrown:{name:"균사 여왕의 관",icon:"👑",slot:"accessory",label:"보스 고유 장신구",mods:{intelligence:2,wisdom:2},price:340,description:"고목 늪의 균사 여왕에게서만 얻는, 균사의 속삭임을 듣는 관.",unique:true},
    emberManeBlade:{name:"잿불 갈기왕의 대검",icon:"⚔️",slot:"weapon",label:"보스 고유 무기",mods:{strength:3,charisma:2},price:380,description:"잿불 갈기왕에게서만 얻는, 불꽃 갈기를 두른 대검.",unique:true},
  ...
  function statBreakdown(base, bonus) { const total = base + bonus; return `<b class="stat-breakdown"><span>기본 <strong>${base}</strong></span><span class="stat-race">종족 ${bonus ? `+${bonus}` : "±0"}</span><span class="stat-total">합계 ${total}</span></b>`; }
  function renderCreator() {
    const race = RACES[state.race]; const total = Object.values(state.stats).reduce((sum, value) => sum + value, 0);
    creator.innerHTML = `<div class="creator-shell"><p class="creator-subtitle">THE FIRST PAGE OF YOUR ADVENTURE</p><h2 class="creator-title">모험가 기록 작성</h2><p class="creator-subtitle">당신의 이름과 타고난 재능이 앞으로의 길을 바꿉니다.</p><div class="creator-layout"><section class="creator-card"><h3>이름</h3><input class="creator-name" maxlength="14" value="${esc(state.name)}" placeholder="모험가의 이름" data-name></section><section class="creator-card"><h3>종족</h3><div class="race-grid">${Object.entries(RACES).map(([id, data]) => `<button type="button" class="race-choice ${state.race === id ? "is-selected" : ""}" data-race="${id}"><strong>${data.name}</strong><small>${data.text}</small></button>`).join("")}</div></section><section class="creator-card"><h3>초상화</h3><p class="portrait-help">기존 초상화를 고르거나, 무작위 외형을 생성하세요.</p><div class="portrait-layout"><div class="portrait" style="${portraitStyle()}"><span class="eye left"></span><span class="eye right"></span></div><div class="appearance-controls">${appearanceRow("피부", "skin", ["#c88864", "#e1ad7c", "#9b624b", "#704134"])}${appearanceRow("머리", "hair", ["#342019", "#7d4b2a", "#d6ad54", "#d6d2c4"])}${appearanceRow("눈", "eyes", ["#6fbd9c", "#6d9bd0", "#b67c5e", "#a594d4"])}<button type="button" class="random-portrait" data-random>✦ 초상화 다시 생성</button></div></div></section><section class="creator-card"><h3>초기 능력치</h3><p class="stat-points">배분 가능 포인트 ${state.points} · 기본 합계 ${total}</p><p class="stat-legend"><span>기본: 직접 배분</span><span>종족: 종족 고유 보정</span><span>합계: 실제 적용 수치</span></p><div class="stat-builder">${Object.keys(STAT_NAMES).map(key => `<div class="stat-row"><label>${STAT_NAMES[key]} <small>${STAT_HELP[key]}</small></label><button type="button" data-minus="${key}">−</button>${statBreakdown(state.stats[key], race.bonus[key] || 0)}<button type="button" data-plus="${key}">+</button></div>`).join("")}</div></section></div><button type="button" class="creator-confirm" data-confirm>이 모험가로 출발하기</button></div>`;
