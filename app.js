...
function render(g) {
  ...
  document.querySelector("#hero-card").innerHTML = `<div><h2 class="hero-name">${p.name} <span class="muted">Lv.${p.level} ${race.name}</span></h2><p class="hero-meta">${race.description}</p>${bar("체력", p.hp, p.maxHp)}${bar("기력", p.stamina, p.maxStamina, "stamina")}${bar("포만감", p.hunger, 100, "hunger")}</div><div class="stats"><strong>힘 ${p.attributes.strength}</strong> · 민 ${p.attributes.agility}<br>지 ${p.attributes.intelligence} · 매 ${p.attributes.charisma}<br>건 ${p.attributes.constitution} · 지혜 ${p.attributes.wisdom}<br><span class="muted">가방 ${p.inventory.length}칸 · XP ${p.xp}</span></div>`;
  ...
  const list = document.querySelector("#action-list"); list.replaceChildren(...actions.map(([label, handler]) => { const button = document.createElement("button"); button.textContent = label; button.addEventListener("click", () => { handler(); g.render(); }); return button; }));
